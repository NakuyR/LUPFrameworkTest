    ...
    {   
        ...
        static IEnumerator Read(UnityWebRequest request, GSTU_Search search, bool containsMergedCells, UnityAction<GstuSpreadSheet> callback)
        {
            ...
            using (request)
            {
                ...
                // 디버깅: API 요청 정보 출력
                Debug.Log($"=== Google Sheets API Request ===");
                ...
                if(string.IsNullOrEmpty(request.downloadHandler.text) || request.downloadHandler.text == "{}")
                {
                    ...
                    Debug.LogError("Possible issues: 1) Sheet name incorrect 2) No data in sheet 3) API key/auth token invalid 4) Spreadsheet not shared/accessible");
                    yield break;
                }
