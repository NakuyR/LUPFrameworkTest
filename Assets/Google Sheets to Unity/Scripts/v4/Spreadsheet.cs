...
namespace GoogleSheetsToUnity
{
    ...
    public class GSTU_SpreadsheetResponce
    {
        ...
        {
            int exclamationIndex = valueRange.range.IndexOf("!");
            if (exclamationIndex == -1)
            {
                return "";
            }
            return valueRange.range.Substring(0, exclamationIndex);
        }
   ...
   /*     public GstuSpreadSheet(GSTU_SpreadsheetResponce data)
        ...
        {
            // 데이터가 비어있는지 확인
            if (data == null || data.valueRange == null || data.valueRange.values == null || data.valueRange.values.Count == 0)
            {
                Debug.LogError("GstuSpreadSheet: No data received from Google Sheets. Check authentication and sheet access.");
                return;
            }

            string startColumn = Regex.Replace(data.StartCell(), "[^a-zA-Z]", "");
            string startRowString = Regex.Replace(data.StartCell(), "[^0-9]", "");

            int startRow = 1; // 기본값
            if (string.IsNullOrEmpty(startRowString) || !int.TryParse(startRowString, out startRow))
            {
                Debug.LogWarning($"StartCell() returned invalid value: '{data.StartCell()}'. Using default row 1.");
                startRow = 1;
            }
