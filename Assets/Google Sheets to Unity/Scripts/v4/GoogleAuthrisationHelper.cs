...
namespace GoogleSheetsToUnity
{
    public class GoogleAuthrisationHelper : MonoBehaviour
    {
        ...
        static void GetAuthComplete(string authToken)
        {
            ...
            Debug.Log("=== GetAuthComplete Called ===");
            Debug.Log("Auth Token = " + authToken);
            Debug.Log("Client ID = " + SpreadsheetManager.Config.CLIENT_ID);
            Debug.Log("Client Secret = " + SpreadsheetManager.Config.CLIENT_SECRET);
            Debug.Log("Client Secret Length = " + SpreadsheetManager.Config.CLIENT_SECRET.Length);

            WWWForm f = new WWWForm();
            f.AddField("code", authToken);
            f.AddField("client_id", SpreadsheetManager.Config.CLIENT_ID);
            f.AddField("client_secret", SpreadsheetManager.Config.CLIENT_SECRET.Trim().Trim('\''));
        ...
        static IEnumerator GetToken(WWWForm f)
        {
            using (UnityWebRequest request = UnityWebRequest.Post("https://accounts.google.com/o/oauth2/token", f))
            {
                ...
                Debug.Log("Access Token: " + SpreadsheetManager.Config.gdr.access_token);
                Debug.Log("Refresh Token: " + SpreadsheetManager.Config.gdr.refresh_token);
